...
function PrepareCouncilMeeting()
  ...
  -- get whole office session member list
	...
	for i=0,SimCnt - 1 do
		...
		if(not officesession_XSimIsInTownhall("Sim"))
	...
	for i=0,SimCnt - 1 do
		ListGetElement("SimOfficeList",i,"Sim")
		if(SimGetOffice("Sim","Office") && XCheckIfOfficeBelongsToTheRightSettlement("settlement","Office"))
...
function PrepareValidSimList(ListAlias)
	...
	for i=0,ListSize - 1 do
		...
		if(not officesession_IsSimValid("ListSim"))
...
function SaveVoteResult(OfficeAlias,WinnerAlias)
 ...
 if(SimGetOffice(WinnerAlias,"CurrentOffice") == 1)
