...
function Init()
	...
	if not GetInsideBuilding("", "Building") or not BuildingIsWorkingTime("Building") then
		MsgDebugMeasure("PayBonus - only available inside a building")
		StopMeasure()
	end
	...
	local MedMoney = ((600*numFound)*Buildinglevel)*(1-(Skill*3))
	local BigMoney = ((1200*numFound)*Buildinglevel)*(1-(Skill*3))
