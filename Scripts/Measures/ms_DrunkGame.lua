function Run()
	...
	local label = ""
    local trunk = {42,44,935,936}
	local alcname, alctextur
	for h=1, 4 do
	    alcname = ItemGetName(trunk[h])
		label = label.."@B["..trunk[h]..",@L_ITEM_"..alcname.."_NAME_+0]"
	end
	...
	repeat
	    ...
	    "@L_MEASURE_DRUNKGAME_STUFF_+6",5,10,20,50,alcstand)
		
		if trunkwahl == 42 then
		    kosten = 5
			gamepoints = 2
			alcpegel = (alcpegel + (Rand(3)+4)) - alcboni
		elseif trunkwahl == 44 then
		    kosten = 10
			gamepoints = 5
			alcpegel = (alcpegel + (Rand(5)+11)) - alcboni	
		elseif trunkwahl == 935 then
		    kosten = 20
			gamepoints = 10
			alcpegel = (alcpegel + (Rand(10)+21)) - alcboni
		elseif trunkwahl == 936 then
		    kosten = 50
			gamepoints = 20
