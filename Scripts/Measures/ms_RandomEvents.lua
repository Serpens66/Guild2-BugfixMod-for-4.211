...
function RandomEventNeutral(SimAlias)
	...
	DynastyGetFamilyMember("PlayerDyn",0,"PlayerSim")
	...
	local mod = (Rand(5)+1)
  local cash = math.ceil((GetMoney("PlayerDyn")/100)*mod)
...
function RandomEventPositiv(SimAlias)
  ...
  elseif RandEvent == 6 then
	    local OfficeHolder = 3
	    while OfficeHolder > 0 do
	      local RandOffice = (Rand(OfficeHolder)+1)
	      GetOfficeTypeHolder("HomeTown",RandOffice,"OfficeHolderSim")
		    if GetDynastyID("PlayerSim") ~= GetDynastyID("OfficeHolderSim") then
